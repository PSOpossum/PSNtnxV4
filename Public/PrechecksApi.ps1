...
.PARAMETER LifecycleV41CommonPrechecksSpec
No description available.
...
PerformPrechecks202Response
#>
function Invoke-PerformPrechecks {
    [CmdletBinding()]
    Param (
        ...
        [PSCustomObject]
        ${LifecycleV41CommonPrechecksSpec},
    ...
    Process {
        ...
        $LocalVarUri = '/lifecycle/v4.1/operations/$actions/prechecks'

        if (!$NTNXRequestId) {
            throw "Error! The required parameter `NTNXRequestId` missing when calling performPrechecks."
        }
        ...
        if (!$LifecycleV41CommonPrechecksSpec) {
            throw "Error! The required parameter `LifecycleV41CommonPrechecksSpec` missing when calling performPrechecks."
        }

        $LocalVarBodyParameter = $LifecycleV41CommonPrechecksSpec | ConvertTo-Json -Depth 100
        ...
        $LocalVarResult = Send-ApiRequest -Method 'POST' `
			...
			-ReturnType "PerformPrechecks202Response" `
