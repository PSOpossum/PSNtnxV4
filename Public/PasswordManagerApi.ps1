...
Initiate password update for a system user
...
.PARAMETER ExtId
External identifier of the system user password.

.PARAMETER NTNXRequestId
A unique identifier that is associated with each request. The provided value must be opaque and preferably in Universal Unique Identifier (UUID) format. This identifier is also used as an idempotence token for safely retrying requests in case of network errors. All the supported Nutanix API clients add this auto-generated request identifier to each request. 

.PARAMETER ClustermgmtV41ConfigChangePasswordSpec
...
function Rename-SystemUserPasswordById {
    ...
    Param (
        ...
        [PSCustomObject]
        ${ClustermgmtV41ConfigChangePasswordSpec},
        [Switch]
        $NoHttpInfo
    )
    ...
    Process {
        ...
        $LocalVarUri = '/clustermgmt/v4.1/config/system-user-passwords/{extId}/$actions/change-password'
        if (!$ExtId) {
            throw "Error! The required parameter `ExtId` missing when calling changeSystemUserPasswordById."
        }
        $LocalVarUri = $LocalVarUri.replace('{extId}', [System.Web.HTTPUtility]::UrlEncode($ExtId))

        if (!$NTNXRequestId) {
            throw "Error! The required parameter `NTNXRequestId` missing when calling changeSystemUserPasswordById."
        }
        $LocalVarHeaderParameters['NTNX-Request-Id'] = $NTNXRequestId

        if (!$ClustermgmtV41ConfigChangePasswordSpec) {
            throw "Error! The required parameter `ClustermgmtV41ConfigChangePasswordSpec` missing when calling changeSystemUserPasswordById."
        }

        $LocalVarBodyParameter = $ClustermgmtV41ConfigChangePasswordSpec | ConvertTo-Json -Depth 100
        ...
        $LocalVarResult = Send-ApiRequest -Method 'POST' `
			...
			-ReturnType "ChangeSystemUserPasswordById202Response" `
...
function Invoke-ListSystemUserPasswords {
    ...
    Process {
        ...
        $LocalVarUri = '/clustermgmt/v4.1/config/system-user-passwords'
        ...
        $LocalVarResult = Send-ApiRequest -Method 'GET' `
			...
			-FormParameters $LocalVarFormParameters `
			-CookieParameters $LocalVarCookieParameters `
			-ReturnType "ListSystemUserPasswords200Response" `
			-IsBodyNullable $false
