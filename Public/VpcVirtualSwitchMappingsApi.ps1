...
.PARAMETER NetworkingV41ConfigVpcVirtualSwitchMapping
Request body for setting VPC for virtual switch mappings traffic config.
...
#>
function New-VpcVirtualSwitchMapping {
    [CmdletBinding()]
    Param (
        ...
        [PSCustomObject[]]
        ${NetworkingV41ConfigVpcVirtualSwitchMapping},
        [Switch]
        $NoHttpInfo
    )
    ...
    Process {
        ...
        $LocalVarUri = '/networking/v4.1/config/vpc-virtual-switch-mappings'

        if (!$NTNXRequestId) {
            throw "Error! The required parameter `NTNXRequestId` missing when calling createVpcVirtualSwitchMapping."
        }
        $LocalVarHeaderParameters['NTNX-Request-Id'] = $NTNXRequestId

        if (!$NetworkingV41ConfigVpcVirtualSwitchMapping) {
            throw "Error! The required parameter `NetworkingV41ConfigVpcVirtualSwitchMapping` missing when calling createVpcVirtualSwitchMapping."
        }

        $LocalVarBodyParameter = ConvertTo-Json @($NetworkingV41ConfigVpcVirtualSwitchMapping) -Depth 100
...
ListVpcVirtualSwitchMappings200Response
#>
function Invoke-ListVpcVirtualSwitchMappings {
    [CmdletBinding()]
    Param (
        [Parameter(Position = 0, ValueFromPipeline = $true, ValueFromPipelineByPropertyName = $true, Mandatory = $false)]
    ...
    Process {
        ...
        $LocalVarResult = Send-ApiRequest -Method 'GET' `
			...
			-FormParameters $LocalVarFormParameters `
			-CookieParameters $LocalVarCookieParameters `
			-ReturnType "ListVpcVirtualSwitchMappings200Response" `
			-IsBodyNullable $false
