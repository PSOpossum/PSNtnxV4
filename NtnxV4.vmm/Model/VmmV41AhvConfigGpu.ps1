...
.PARAMETER NumVirtualDisplayHeads
...
function Initialize-NtnxV4VmmV41AhvConfigGpu {
    ...
    Param (
        ...
        [ValidateSet("NVIDIA", "INTEL", "AMD", "UNKNOWN", "REDACTED")]
        [PSCustomObject]
        ${Vendor},
    ...
    Process {
        ...
        if (!$GuestDriverVersion -and $GuestDriverVersion.length -gt 128) {
            throw "invalid value for 'GuestDriverVersion', the character length must be smaller than or equal to 128."
        }
...
Convert from JSON to VmmV41AhvConfigGpu<PSCustomObject>
...
function ConvertFrom-NtnxV4JsonToVmmV41AhvConfigGpu {
    ...
    Process {
        ...
        if (!([bool]($JsonParameters.PSobject.Properties.name -match "numVirtualDisplayHeads"))) { #optional property not found
            $NumVirtualDisplayHeads = $null
        } else {
            $NumVirtualDisplayHeads = $JsonParameters.PSobject.Properties["numVirtualDisplayHeads"].value
        }
