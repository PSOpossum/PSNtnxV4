#
# Nutanix Cluster Management APIs
# Manage Hosts, Clusters and other Infrastructure.
# Version: 4.1.1
# Generated by OpenAPI Generator: https://openapi-generator.tech
#

<#
.SYNOPSIS

No summary available.

.DESCRIPTION

SMTP servers on a cluster. This is part of payload for cluster update operation only.

.PARAMETER EmailAddress
SMTP email address.
.PARAMETER Server
No description available.
.PARAMETER Type
No description available.
.OUTPUTS

ClustermgmtV41ConfigSmtpServerRef<PSCustomObject>
#>

function Initialize-ClustermgmtV41ConfigSmtpServerRef {
    [CmdletBinding()]
    Param (
        [Parameter(Position = 0, ValueFromPipelineByPropertyName = $true)]
        [ValidatePattern("^([a-zA-Z0-9_\-\.]+)@([a-zA-Z0-9_\-\.]+)\.([a-zA-Z]{2,5})$")]
        [String]
        ${EmailAddress},
        [Parameter(Position = 1, ValueFromPipelineByPropertyName = $true)]
        [PSCustomObject]
        ${Server},
        [Parameter(Position = 2, ValueFromPipelineByPropertyName = $true)]
        [ValidateSet("PLAIN", "STARTTLS", "SSL", "UNKNOWN", "REDACTED")]
        [PSCustomObject]
        ${Type}
    )

    Process {
        'Creating PSCustomObject: clustermgmt => ClustermgmtV41ConfigSmtpServerRef' | Write-Debug
        $PSBoundParameters | Out-DebugParameter | Write-Debug

        if ($null -eq $EmailAddress) {
            throw "invalid value for 'EmailAddress', 'EmailAddress' cannot be null."
        }

        if ($null -eq $Server) {
            throw "invalid value for 'Server', 'Server' cannot be null."
        }


        $PSO = [PSCustomObject]@{
            "emailAddress" = ${EmailAddress}
            "server" = ${Server}
            "type" = ${Type}
        }


        return $PSO
    }
}

<#
.SYNOPSIS

Convert from JSON to ClustermgmtV41ConfigSmtpServerRef<PSCustomObject>

.DESCRIPTION

Convert from JSON to ClustermgmtV41ConfigSmtpServerRef<PSCustomObject>

.PARAMETER Json

Json object

.OUTPUTS

ClustermgmtV41ConfigSmtpServerRef<PSCustomObject>
#>
function ConvertFrom-JsonToClustermgmtV41ConfigSmtpServerRef {
    Param(
        [AllowEmptyString()]
        [string]$Json
    )

    Process {
        'Converting JSON to PSCustomObject: clustermgmt => ClustermgmtV41ConfigSmtpServerRef' | Write-Debug
        $PSBoundParameters | Out-DebugParameter | Write-Debug

        $JsonParameters = ConvertFrom-Json -InputObject $Json

        # check if Json contains properties not defined in ClustermgmtV41ConfigSmtpServerRef
        $AllProperties = ("emailAddress", "server", "type")
        foreach ($name in $JsonParameters.PsObject.Properties.Name) {
            if (!($AllProperties.Contains($name))) {
                throw "Error! JSON key '$name' not found in the properties: $($AllProperties)"
            }
        }

        If ([string]::IsNullOrEmpty($Json) -or $Json -eq "{}") { # empty json
            throw "Error! Empty JSON cannot be serialized due to the required property 'emailAddress' missing."
        }

        if (!([bool]($JsonParameters.PSobject.Properties.name -match "emailAddress"))) {
            throw "Error! JSON cannot be serialized due to the required property 'emailAddress' missing."
        } else {
            $EmailAddress = $JsonParameters.PSobject.Properties["emailAddress"].value
        }

        if (!([bool]($JsonParameters.PSobject.Properties.name -match "server"))) {
            throw "Error! JSON cannot be serialized due to the required property 'server' missing."
        } else {
            $Server = $JsonParameters.PSobject.Properties["server"].value
        }

        if (!([bool]($JsonParameters.PSobject.Properties.name -match "type"))) { #optional property not found
            $Type = $null
        } else {
            $Type = $JsonParameters.PSobject.Properties["type"].value
        }

        $PSO = [PSCustomObject]@{
            "emailAddress" = ${EmailAddress}
            "server" = ${Server}
            "type" = ${Type}
        }

        return $PSO
    }

}

