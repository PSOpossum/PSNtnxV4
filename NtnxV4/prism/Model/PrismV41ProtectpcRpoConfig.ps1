...
function Initialize-PrismV41ProtectpcRpoConfig {
    ...
    Process {
        'Creating PSCustomObject: prism => PrismV41ProtectpcRpoConfig' | Write-Debug
...
function ConvertFrom-JsonToPrismV41ProtectpcRpoConfig {
    ...
    Process {
        'Converting JSON to PSCustomObject: prism => PrismV41ProtectpcRpoConfig' | Write-Debug
        $PSBoundParameters | Out-DebugParameter | Write-Debug

        $JsonParameters = ConvertFrom-Json -InputObject $Json

        # check if Json contains properties not defined in PrismV41ProtectpcRpoConfig
