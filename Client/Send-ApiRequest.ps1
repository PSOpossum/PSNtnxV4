function Send-ApiRequest {
    ...
    $DefaultHeaders = $PrismSession["DefaultHeaders"]

    # Basic Authorization method
    if ($PrismSession["AuthorizationType"] -eq "Basic") {
        $HeaderParameters["Authorization"] = $PrismSession["Authorization"]
    }

    # Apikey Authorization method
    if ($PrismSession["AuthorizationType"] -eq "ApiKey") {
        if ($PrismSession["ApiKeyPrefix"] -and $PrismSession["ApiKeyPrefix"]["X-Ntnx-Api-Key"]) {
            $apiKeyPrefix = $PrismSession["ApiKeyPrefix"]["X-Ntnx-Api-Key"]
        } else {
            $apiKeyPrefix = ""
        }
    ...
    $SkipCertificateCheck = $PrismSession["SkipCertificateCheck"]
    $Proxy = $PrismSession["Proxy"]
